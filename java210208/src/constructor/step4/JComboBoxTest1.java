...
//생성자 파라미터 활용 연습
//인스턴스화 이전에 반드시 생성자 확인할 것. API(Applicatioin Programming Interface)
/* new JFrame();
 * new JFrame("타이틀");
 * new JComboBox();
 * new JComboBox(String[]);
 * new JComboBox(String);    ->불법
 * 대분류   가 결정 되어야
 * 중분류   가 결정 되어야
 * 소분류
 * 
...
public class JComboBoxTest1 extends JFrame implements ItemListener{
	...
	void initDisplay() {
		...
		this.setTitle("대중소분류 실습이다옹");
		this.setSize(400, 500);
		this.setVisible(true);
	}
	//메인메소드 선언
	public static void main(String[] args) {
		new JComboBoxTest1();//생성자를 경유하고 생성자에서 화면처리하는 메소드 호출해보기 - 시점문제에 대해 같이 고민.
	}
	...
	public void itemStateChanged(ItemEvent ie) { //콜백메소드이다. -시스템레벨에서 이벤트가 감지 되었을 때 호출됨.
		...
		if(obj == jcb_top) {
			...
			if(ie.getStateChange() == ItemEvent.SELECTED) { //타입.속성명 - 하나다.
				...
				jcb_mid.removeAllItems(); //기존에 totals에 있떤 전체를 삭제하고 새로운 아이템으로 변경 준비
				if("가전".equals(top)) {
			    //if(top.equals("가전")) { 이렇게 쓰기 ㄴㄴ. 항상 상수를 앞에
