...
public class TalkServerThread extends Thread {
	TalkServer			ts 		= null;
	Socket 				client 	= null;
	ObjectOutputStream 	oos 	= null; //이모티콘 등 다양한 타입을 보내기 위함
	...
	public TalkServerThread(TalkServer talkServer) {
		...
		try {
			oos = new ObjectOutputStream(client.getOutputStream());//홀수자리 소켓 - 쓰기 역할
			...
			for(TalkServerThread tst:ts.globalList) {
				...
				this.send(100+"#"+tst.nickName);//클라이언트 스레드의 정보들을 받기 ====스레드에 따라 변경된다
			}
