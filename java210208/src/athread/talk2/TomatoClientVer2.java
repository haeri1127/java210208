...
public class TomatoClientVer2 extends JFrame implements ActionListener {
	...
	public void initDisplay() {
		...
		jp_second_south.setLayout(new GridLayout(3,2)); //row가 3개, col이 2개
	...
	public static void main(String args[]) {
		...
		//화면 부름
		tc.initDisplay();
		//소켓 생성 - TS쪽 ServerSocket감지 - 일반소켓 전달됨. -서버측의 run메소드 안에서 - TomatoServerThread생성
	...
	public void init() {
		try {
			...
			//TS ServerSocket감지 ->clinet = server.accpet(); //클라이언트 소켓에 대한 정보 갖음
			//홀수 소켓에 대한 처리
			...
			//서버에 말을 한 후 들을 준비를 한다. -대기 탄다 -메세지가 날라오면 듣기 -프로토콜을 비교해야한다.
			//프로토콜 설계하기  -ERD 그린다. -데이터 클래스 설계 -List, Map 단위테스트까지 너가 해주면 ^^웃음
		...
		if(jbtn_one == obj) {
			...
			try {
				oos.writeObject(Protocol.MESSAGE
								...
								+Protocol.seperator+msg); //200#하하#오늘 스터디 할까?
				jtf_msg.setText("");
			} catch (Exception e) {
			...
			if(afterName == null || afterName.length()<3) {
				JOptionPane.showMessageDialog(this,"다시 확인해주세요","INFO",JOptionPane.INFORMATION_MESSAGE);
				return;
			}
			try {
			...
			try {
				oos.writeObject(Protocol.ROOM_OUT
								+Protocol.seperator+this.nickName); //190#하하
				//자바가상머신과 연결고리 끊기
				System.exit(0);
			} catch (Exception e) {
				// TODO: handle exception
