...
public class SalaryMgr implements ActionListener{
	//선언부
	JFrame 		jf_sal = null;
	//DefaultTableModel(DataSet역할:data[][],header[])+JTalbe=> 테이블
	String 				cols[]		= 	{"사원명","부서명"};
	...
	public SalaryMgr() {
		...
		for(int i=0; i<jbtns_label.length;i++) {
			...
			jbtns[i].addActionListener(this);//this는 현재 활성화된 객체 주소번지 SaralyMgr
		}
		initDisplay();
	}
	...
	public void initDisplay() {
		System.out.println("initDisplay호출 성공");//API활용 능력 키워준다.
	...
	//	if(jbtns[4]==obj) {
			...
			//자바 가상 머신과의 연결고리 끊어줌. -가비지 컬렉터에 의해서 Candidate 상태로 빠짐
			System.exit(0);
			//System.gc();//쓰레리값 청소해주세요. 호출하더라고 스레드에 의해 순서대로 처리됨. 즉시 처리 안됨.
		}
