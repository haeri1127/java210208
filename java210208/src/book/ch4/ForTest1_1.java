...
//메인메소드를 선언하려면 반드시 클래스 선언이 먼저 이다. -처리 주체가 JVM 즉 가상 머신이다. -일꾼이 가상머신이다.
//모든 코딩의 기준은 자바가상 머신이 해석할 수 있는 방식으로 코딩이 전개되어야 한다.
//클래스 선언하기는
//접근제한자, class라는 예약어, 그리고 클래스명 순서로 선언됩니다.
//접근제한자 public, private, 여기 나와있는 사람들끼리만 - protected, 같은 폴더(패키지)에 있는 클래스끼리는 공유가능하다.-friendly상태이다 class A{}
//클래스 내부는 선언부와 메소드 선언부로 나누어서 코딩을 전개해본다.
public class ForTest1_1 {
	//선언부 - 전역변수이다 초기화를 생략할 수 있다. 생성자가 있는데 역할이 전변의 초기화를 담당한다.
	//지역변수는 선언이 불가함- 반드시 메소드 안에 있어야 한다.
	//if문이나 for문은 사용이 불가능하다. - 실행문갖고 있다.  hap = hap+i;//실행문 공식 알고리즘
	//선언과 초기화를 분리할 수 없다. - 컴파일이 안되니까 실행할 수 없어.
	//int i; i =10; ->불가능. 메소드 안에서는 된다. 선언부에서만 안된다.
	//if(true){}  //->선언부에 실행문을 적을 수 없다. 메소드안에 써야함 - 재사용성을 높이기 위해서
	...
	//이중으로 관리되지 않도록 해야한다.
	////////////////////////////////////end of 선언부
	//메소드를 선언한 영역 - 변수를 선언한 부분과 구분해야 한다.
