 ...
 */
	...
	public String[] getZDOList() {
		...
		try {
			...
			rs = pstmt.executeQuery();
			Vector<String> v = new Vector<>();
			List<String> v2 = new Vector<>();   //위 벡터와 이 리스트의 차이에 대해서 생각해보자. 중요!
			while(rs.next()) {
		...
		try {
			...
			if(v.size()>0) {
				...
				for(int x=0;x<v.size();x++) {
					...
					//안돼 ArrayList<Object> 	oneRow2 = new ArrayList<>();
					//안돼 List<Object>		oneRow3	= new ArrayList<>();
					//안돼 List<Object>		oneRow4	= new Vector<>();
					oneRow.add(0,zVOS[x].getZipcode());
					oneRow.add(1,zVOS[x].getAddress());
					oneRow.add(2,zVOS[x].getAddress());
					//int y = 0;
					//oneRow.add(++y,zVOS[x].getZipcode());
					//oneRow.add(++y,zVOS[x].getAddress());
					//oneRow.add(++y,zVOS[x].getAddress());
...
//오라클에서 조회된 결과를 담기는 부분이 바로 여기다.....
	...
	public void itemStateChanged(ItemEvent ie) {
		//Object obj = it.getSource().getClass
		...
		if(e.getClickCount()==2) {
			...
			for(int i=0; i<dtm_zipcode.getRowCount(); i++) {
				if(jtb_zipcode.isRowSelected(i)) {
					...
					memberShip.jtf_zipcode.setText(address);
				}
			}////////////////end of for
		}
